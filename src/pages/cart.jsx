...
const Cart = ({ data }) => {
  ...
  const { Price, Saving, Item, cart, Pincode, wishlistItem } = useSelector(
    (state) => {
      return {
    ...
    if (maindata.length != 0) {
      for (let i = 0; i < maindata.length; i++) {
        ...
        if (maindata.length != 0 && maindata[i].originalPrice != "") {
          sum2 +=
            (Number(maindata[i].originalPrice) -
              Number(maindata[i].currentPrice)) *
            ref.current[i].value;
        }
      }
              ...
              <div className={styles.singleCart} key={el.id}>
                ...
                <div className={styles.singleCart2}>
                  ...
                  <p>
                    ...
                    <span className={styles.span2}>₹{~~el.originalPrice}</span>{" "}
